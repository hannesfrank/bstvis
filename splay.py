	...
	def _splay(self, p):
		while p.parent != None:
			...
			if p.parent.parent == None:
				# zig: one step left
				p.rotate()
			elif p == p.parent.left and p.parent == p.grand_parent.left \
			  or p == p.parent.right and p.parent == p.grand_parent.right:
				# zig zig
				p.parent.rotate()
				p.rotate()
			elif p == p.parent.left and p.parent == p.grand_parent.right \
			  or p == p.parent.right and p.parent == p.grand_parent.left:
			  	# zig zag
			  	p.rotate()
			  	p.rotate()
